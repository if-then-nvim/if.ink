...
---@field mode? encre.Mode|encre.Mode[] how to render a match; a list applies several at once
...
---@alias encre.HighlightCache table<string, string> cache key -> highlight group name
---@alias encre.ColorMap table<string, string> lowercased colour name -> six hex digits
---@alias encre.AttachedMap table<integer, encre.Options> buffer -> options it was attached with
---@alias encre.LspColorCache table<integer, table<integer, encre.Match[]>> buffer -> row -> matches
---@alias encre.TimerMap table<integer, uv.uv_timer_t> buffer -> debounce timer
---@alias encre.AutocmdMap table<integer, integer[]> buffer -> autocmd ids
